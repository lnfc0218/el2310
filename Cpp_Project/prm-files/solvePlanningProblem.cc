...
std::string probSpecFile = "problem1.txt";
  ...
  // std::string worldModel="SingleRectangleWorld";
  ...
  else if(worldModel=="SingleRectangleWorld")
    {
      world=new SingleRectangleWorld;
    }
  else if(worldModel=="MyWorld")
    {
      world=new MyWorld;
    }
  ...
  if (!world->readObstacles(fs)) {//initialization actually starts here!
    std::cerr << "Failed to read obstacle, exiting\n";
    return -1;
  }
  fs.close();//close the problem specification file
